...
object WARC {
  def load(sc: SparkContext, path: String, name: String) : DataFrame = {
    ...
    val warcEntriesMap = warcJoined.flatMap(t => (t._2._1 to t._2._2).map(i => (i, t._1))).cache

    warcEntriesMap.count()
    ...
    val df = warcPaged.toDF.cache()

    df.registerTempTable(name)
    df.count()

    warcEntriesMap.unpersist(true)
    warcIdx.unpersist(true)
